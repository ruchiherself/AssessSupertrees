...
bool stream2tree(std::istream &is, TREE &tree, idx2name &names, WEIGHTS &weights, boost::unordered_map<int,std::string> &mapping, int nm, std::string &cmt) {
    ...
    std::stack<unsigned long int> parents;
    ...
    for (;;) {
        ...
        if (c == ')') { // subtree completed
            //std::cout<<"<bf>";
            if(parents.empty()) std::cout<<"Empty";
...
//             else ERROR_return("cannot read weight/branch length value " << input.getLastPos());
            ...
            if(nm==1) { //store names
                ...
                std::string subst = name.substr(pos+4);
                //std::cout<<" num:"<<subst;
                int numb = std::atoi(subst.c_str());
                if(numb==0)  ERROR_exit("Wrong conversion"<<numb<<" "<<name<<" "<<subst);

                if(mapping.find(numb) == mapping.end())
                    mapping[numb] = name;
                else
                    ERROR_exit("Label repeated: "<<numb<<" "<<name);
                names.insert(idx2name::value_type(lin, name));
            }
            else {
                unsigned int pos = name.find_last_of("_");
                std::string subst = name.substr(pos+1);
                //std::cout<<" num:"<<subst;
                int numb = std::atoi(subst.c_str());
                if(numb==0)  ERROR_exit("Wrong conversion"<<numb<<" "<<name);
                if(mapping.find(numb) == mapping.end())  ERROR_exit("Label not found"<<" "<<numb<<" "<<name);
                names.insert(idx2name::value_type(lin, mapping[numb]));
            }
        }
        if (!input.nextAnyChar(c)) return false;
    }
...
template<class TREE>
inline bool stream2tree(std::istream &is, TREE &tree, idx2name &names, boost::unordered_map<int,std::string> &map, int nm, std::string &cmt) {
    idx2weight weights;
    return stream2tree(is, tree, names, weights, map, nm, cmt);
}
