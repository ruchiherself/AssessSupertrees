...
public: inline bool is_leaf(const unsigned int v) { return (degree(v) == 1); }  //Ruchi: 8th May
    ...
    //delete degree 2 node v
    public: inline bool deg2_delete(unsigned int v) {
        //std::cout<<"--"<<v;
        unsigned int lf;
        ...
        if(ch.size() != 2) ERROR_exit("Node is not degree 2!"<<ch.size());
        if(is_leaf(ch[0]))
            lf = ch[0];
        else
            lf = ch[1];
    ...
    //delete degree 2 node v
    public: inline bool nphy_delete(unsigned int v) {
        //std::cout<<"--"<<v;
        //unsigned int lf;
