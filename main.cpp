 ...
 * output: a file contianing interesting parameters
 * 
 * File:   main.cpp
...
int main(int ac, char* av[]) {
    ...
    bool taxonomy = false;
    {
        ...
        if (a.existArg2("-h","--help")) {
            ...
            MSG(" -c 1 = stat, 2 = triplet count, 3 = RF distance, and 4 = assess");
            MSG(" --tax");
...
//    std::vector<unsigned int> d2_nodes;
//    for (aw::Tree::iterator_dfs vl=s_tree.begin_dfs(),vEE=s_tree.end_dfs(); vl!=vEE; ++vl) {
//         if(vl.idx==0) continue;
//
//         switch (vl.direction) {
//            case aw::PREORDER: {
//                if(s_tree.is_leaf(vl.idx) && s_tree.degree(vl.parent)==2 && s_tree.root!=vl.parent) {
//                     d2_nodes.push_back(vl.parent);
//                     //output<<" "<<vl.parent;
//                }
//            }
//         }
...
//
//
//     BOOST_FOREACH(const unsigned int &i,d2_nodes) {
//         //std::cout<<" -"<<i;
//         if(!s_tree.deg2_delete(i)) ERROR_exit("Error deleting edge");
//         //s_taxa[i] = s_taxa[lf];
//     }
//
//     aw::tree2newick(output,s_tree,s_taxa);
//
//     return 0;
    ...
    {   //map strees labels
        ...
        //if(n.size()<taxamap.size())  ERROR_exit("Error: Supertree doesn't have all labels!!");
        taxamap.insert(n);
        s_nmap.create(n,taxamap);
    }

    //strip extra leaves from the input trees for RF and assess
    if(option==3 || option==4) {        
        for (unsigned int i=0,iEE=g_trees.size(); i<iEE; ++i) {
            ...
            TREE_FOREACHLEAF(v,g_trees[i]) {
                leaves0++;
                if(g_trees[i].degree(v)!=1)  MSG("ERROR");
                unsigned int gid_v = g_nmaps[i].gid(v);
                if(!s_nmap.exists(gid_v)) {
                    leaves_trim.push_back(v);
                }
            }
            if(leaves_trim.size()>0) MSG("Tree: "<<i<<", trimming "<<leaves_trim.size());
    ...
    if(option==1) {  //STATS OF ALL INPUT TREES .....
        ...
        output<<"OpenTree leaves: "<<s_cnt<<", unresolved nodes: "<<unr_cnt<<", resolved nodes: "<<r_cnt<<", degree two nodes: "<<no_phy_cnt<<", knees: "<<knee<<std::endl;
        ...
        for(unsigned int i=0; i<g_trees.size(); ++i) {
            ...
            for (aw::Tree::iterator_dfs vl=g_trees[i].begin_dfs(),vEE=g_trees[i].end_dfs(); vl!=vEE; ++vl) {
                 if(vl.idx==0) {
                     //MSG("Taxonomy root's degree: "<<g_trees[0].degree(vl.idx));
                     continue;
                 }

                 //if(vl.parent==0)
                     //MSG(" label:"<<g_taxa[0][vl.idx]);

                 switch (vl.direction) {
                    case aw::PREORDER: {
                        if(g_trees[i].is_leaf(vl.idx)) {
                            if(g_trees[i].degree(vl.parent)==2)
                                knee++;
                            t_cnt++;
                            continue;
                        }
            ...
            if(taxonomy && i==0)
                output<<" Taxonomy leaves: "<<t_cnt<<", unresolved nodes: "<<unres_cnt<<", resolved nodes: "<<r_cnt<<", degree two nodes: "<<n_phy_cnt<<", knee: "<<knee<<std::endl;
            else
                output<<" Study tree "<<i<<" leaves: "<<t_cnt<<", unresolved nodes: "<<unres_cnt<<", resolved nodes: "<<r_cnt<<", degree two nodes: "<<n_phy_cnt<<", knee: "<<knee<<std::endl;
        }

    }
    if(option==2) { //RESOLVED TRIPLETS ---------------------------------
            ...
            TREE_POSTORDER2(v,s_tree) {
                if (s_tree.is_leaf(v.idx)){
                   s_tree.update_clst(v.idx,1);
                    ...
                    //if(s_tree.root==v.parent)  MSG(" *"<<s_tree.return_clstSz(v.idx));
                }
            }
        }
    ...
    } //DONE counting triplets -------
    else if(option == 3) { //Computing RF DISTSCNER
       ...
       {  //store lcas
        ...
        for (unsigned int i=0,iEE=g_trees.size(); i<iEE; ++i) {
            ...
            MSG("s_nds "<<s_nds<<" spt "<<spt<<" g_nodes[i] "<<g_nodes[i]);
            ...
            output<<" ATE: "<< ((s_nds - spt + g_nodes[i]-spt)*100)/(s_nds+g_nodes[i])<<std::endl;
        ...
        for (unsigned int i=0,iEE=g_trees.size(); i<iEE; ++i) {
            ...
            //int g_leaves = g_nmaps[i].unq_leaves();
            std::cout<<"g_leaves:"<<g_leaves;
        ...
        TREE_POSTORDER2(v,s_tree) {
            if((s_tree.return_clstSz(v.idx)!=1) && (s_tree.root!=v.idx)) {
                ...
                //if((V==0.0 && V_plus==0.0 && V_minus==0.0 && ws[v.idx]==0.0 && ss[v.idx]==0.0))
        ...
        MSG("Nodes that are conflicted by two or more trees: "<<conflict_count);
